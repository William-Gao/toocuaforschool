...
def train(model, processor, ds, train_items, cfg: TrainCfg):
    ...
    # Restore checkpoints from Drive if local dir is empty (e.g. after disconnect)
    if cfg.drive_dir and not cfg.smoke and _has_ckpts(cfg.drive_dir) and not _has_ckpts(cfg.output_dir):
        print(f"[florence] Restoring checkpoints from Drive → {cfg.output_dir}")
        import shutil
        for item in os.listdir(cfg.drive_dir):
            s = os.path.join(cfg.drive_dir, item)
            d = os.path.join(cfg.output_dir, item)
            if os.path.isdir(s) and item.startswith("checkpoint-"):
                shutil.copytree(s, d)
            elif os.path.isfile(s):
                shutil.copy2(s, d)
        print(f"[florence] Restored. Contents: {os.listdir(cfg.output_dir)}")
...
def evaluate(model, processor, eval_items, ds, max_new_tokens=8,
    ...
    for i, raw_item in enumerate(eval_items):
        ...
        # Progress display
        if _total > 10 and (i + 1) % max(1, _total // 20) == 0 or i == _total - 1:
            _elapsed = time.time() - t0
            _rate = (i + 1) / _elapsed
            _eta = (_total - i - 1) / _rate
            _bar_filled = int(30 * (i + 1) / _total)
            _bar = "█" * _bar_filled + "░" * (30 - _bar_filled)
            try:
                from IPython.display import clear_output
                clear_output(wait=True)
            except ImportError:
                pass
            print(f"Florence-2 fine-tuned eval ({decode_mode})")
            print(f"{_bar} {i+1}/{_total}")
            _parsed = sum(1 for r in rows if r.get('pred_x') is not None)
            print(f"parse rate: {_parsed/(i+1)*100:.0f}%  elapsed: {_elapsed:.0f}s  ETA: {_eta:.0f}s")
